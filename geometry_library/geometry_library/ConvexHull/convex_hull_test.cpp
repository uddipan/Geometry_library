...
// Generate cnt random points and populate allPts vector with random points in
// [-1,1]
...
bool ConvexHulltest(int method) {
  std::vector<Vec2> allPts;
  generate_random(allPts, kPointCntLow);

  std::vector<Vec2> CHull = ConvHull(allPts, method);
  if (!IsHullCorrect(allPts, CHull)) {
    return false;
  }

  generate_random(allPts, kPointCntMedium);
  CHull = ConvHull(allPts, method);
  if (!IsHullCorrect(allPts, CHull)) {
    return false;
  }

  generate_random(allPts, kPointCntHigh);
  CHull = ConvHull(allPts, GRAHAM_SCAN);
  if (!IsHullCorrect(allPts, CHull)) {
    return false;
  }

  generate_random(allPts, kPointCntDense);
  CHull = ConvHull(allPts, method);
  if (!IsHullCorrect(allPts, CHull)) {
    return false;
  }
...
TEST(ConvexHullTest, AllMethods) {
  EXPECT_TRUE(ConvexHulltest(GIFT_WRAP));
  EXPECT_TRUE(ConvexHulltest(QUICK_HULL));
  EXPECT_TRUE(ConvexHulltest(GRAHAM_SCAN));
}
