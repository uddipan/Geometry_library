...
inline double LengthSquared(const Vec2 &A) { return A.x * A.x + A.y * A.y; }

inline double Length(const Vec2 &A) { return sqrt(LengthSquared(A)); }
...
inline double operator^(const Vec2 &A,
                        const Vec2 &B)  // Z-component of Cross product.
{
